...
def compile_shader_with_gs(vs, fs, gs):
    vert_shader = load_shader(vs)
    geom_shader = load_shader(gs)
    frag_shader = load_shader(fs)

    shader = OpenGL.GL.shaders.compileProgram(OpenGL.GL.shaders.compileShader(vert_shader, GL_VERTEX_SHADER),
                                              OpenGL.GL.shaders.compileShader(geom_shader, GL_GEOMETRY_SHADER),
                                              OpenGL.GL.shaders.compileShader(frag_shader, GL_FRAGMENT_SHADER))
    return shader


def compile_shader_with_ts(vs, fs, tcs, tes):
    vert_shader = load_shader(vs)
    tcs_shader = load_shader(tcs)
    tes_shader = load_shader(tes)
    frag_shader = load_shader(fs)

    shader = OpenGL.GL.shaders.compileProgram(OpenGL.GL.shaders.compileShader(vert_shader, GL_VERTEX_SHADER),
                                              OpenGL.GL.shaders.compileShader(tcs_shader, GL_TESS_CONTROL_SHADER),
                                              OpenGL.GL.shaders.compileShader(tes_shader, GL_TESS_EVALUATION_SHADER),
                                              OpenGL.GL.shaders.compileShader(frag_shader, GL_FRAGMENT_SHADER))
    return shader


class ShaderProgram:
    def __init__(self, vs, fs, gs=None, tcs=None, tes=None):
        if gs is None and tcs is None and tes is None:
            self.shader = compile_shader(vs, fs)
        elif gs is None:
            self.shader = compile_shader_with_ts(vs, fs, tcs, tes)
        else:
            self.shader = compile_shader_with_gs(vs, fs, gs)
